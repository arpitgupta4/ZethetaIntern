...
export const Step3KYC: React.FC = () => {
  ...
  const { register, handleSubmit, watch, formState: { errors } } = useForm<Step3Data>({
    ...
    defaultValues: { panNumber: formData?.panNumber || '', aadhaarNumber: formData?.aadhaarNumber || '' },
  });
  ...
  const onSubmit = (data: Step3Data) => {
    if (!formData.panVerified || !formData.aadhaarVerified) {
      alert("Please verify both documents."); return;
    }
    updateFormData(data); nextStep();
  };
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 w-full max-w-md mx-auto">
      <div className="bg-white p-4 rounded-xl border border-gray-200">
        ...
        <div className="flex gap-2">
          <input type="text" {...register('panNumber')} disabled={formData?.panVerified} className="w-full p-3 border border-gray-300 rounded-lg uppercase" />
          <button type="button" onClick={verifyPanAPI} disabled={formData?.panVerified || !!errors.panNumber || !panValue} className="px-4 py-2 bg-slate-800 text-white rounded-lg flex items-center justify-center min-w-[100px]">
      ...
      <div className="bg-white p-4 rounded-xl border border-gray-200">
        ...
        <div className="flex gap-2">
          <input type="text" {...register('aadhaarNumber')} maxLength={12} disabled={formData?.aadhaarVerified} className="w-full p-3 border border-gray-300 rounded-lg" />
          <button type="button" onClick={verifyAadhaarAPI} disabled={formData?.aadhaarVerified || !!errors.aadhaarNumber || !aadhaarValue} className="px-4 py-2 bg-slate-800 text-white rounded-lg flex items-center justify-center min-w-[100px]">
